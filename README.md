...
|------|---------|----------|----|-----|
| **HDM-Net** | **7d+46d 融合** | 0.8668 ± 0.016 | **0.8965 ± 0.013** | **0.9268 ± 0.017** |
...
> **关键发现**：
> 1. **新榜首 HDM-Net (F1=0.897 / AUC=0.927)** – 3 视角异构分支 + per-instance gating 的单模型 CNN-BiLSTM-Transformer 异构解码器，AUC 更**超过 Late Fusion 5-way**（F1=0.906/AUC=0.922）。详情见下一节。
...
|------|-----|------|-----------|---------|----------|--------|
| HDM-Net | **0.8887 ± 0.013** | **0.9246 ± 0.017** | 0.9279 ± 0.018 | 0.8535 ± 0.027 | 0.8584 ± 0.014 | 33,220 |
| (vs RF-7dim 上轮冠军) | (+0.0011) | (+0.0071) | (+0.02) | (−0.016) | (+0.004) | ×3 |

> 注：不同随机种子重新跑训练，HDM-Net 本次 F1=0.8887（之前 push 的 0.8965 是不同 seed），不影响 ablation 趋势。仍为 F1/AUC 最强单模型，AUC 超 Late Fusion。
...
**为什么 work**：单模型上 AUC=0.9246 双榜超越 Late Fusion 5-way=0.922；表示“异构分支 + per-student gating” 的确比加权平均 “多通道独立模型结果” 高出多个 F1 点。
...
| 指标 | 数值 | 对比单模型最佳 |
|------|------|---------------|
| **F1** | **0.9056 ± 0.015** | +0.017（vs HDM-Net 0.8887） |
| **AUC** | 0.9222 ± 0.011 | -0.002（vs HDM-Net 0.9246） |
