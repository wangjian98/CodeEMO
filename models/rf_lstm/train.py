"""
RF-Enhanced BiLSTM 训练脚本 - 5-fold CV, failed=1 口径
"""
...
from models.rf_lstm.model import RFEnhancedBiLSTM, count_parameters
...
def make_rf_oof_probs(X7, y, n_splits=5, seed=42):
    """OOF RF probs (will be used as frozen RF-LSTM auxiliary input)."""
    n = len(y)
...
def train_one_fold(model, optimizer, scheduler, bce, device,
    ...
    for ep in range(epochs):
        ...
        for i in range(0, n, batch_size):
            ...
            logits = model(x_seq_tr_t[b], rf_tr_t[b])
        ...
        model.eval()
        with torch.no_grad():
            v_logits = model(x_seq_va_t, rf_va_t)
    ...
    model.eval()
    with torch.no_grad():
        v_logits = model(x_seq_va_t, rf_va_t).cpu().numpy()
    return 1.0 / (1.0 + np.exp(-v_logits))


def main(output_dir=None, folds=5, epochs=80, batch_size=32, patience=10, seed=42):
    if output_dir is None:
        output_dir = os.path.join(_PROJECT_ROOT, 'outputs', 'unified_compare', 'rf_lstm')
    os.makedirs(output_dir, exist_ok=True)

    print("=" * 72)
    print("  RF-Enhanced BiLSTM: Architecture-level RF + LSTM fusion")
    print("=" * 72)
    ...
    # OOF RF probs (frozen auxiliary input)
    print("\n[Step 1] Computing 5-fold OOF RF probs (frozen feature)...")
    rf_probs = make_rf_oof_probs(X7, y_eval, n_splits=folds, seed=seed)
    print(f"  RF probs shape={rf_probs.shape}")

    # 5-fold CV
    print(f"\n[Step 2] RF-Enhanced BiLSTM 5-fold CV (epochs={epochs}, batch={batch_size}, patience={patience})")
    ...
    for fi, (tr, va) in enumerate(skf.split(X7, y_eval), 1):
        # standardize 46-dim features
        scaler = StandardScaler()
        x_seq_tr = scaler.fit_transform(X_full[tr]).astype(np.float32)
        x_seq_va = scaler.transform(X_full[va]).astype(np.float32)
        rf_tr = rf_probs[tr].astype(np.float32)
        rf_va = rf_probs[va].astype(np.float32)

        # init model
        torch.manual_seed(seed + fi); np.random.seed(seed + fi)
        model = RFEnhancedBiLSTM(seq_dim=46, rf_dim=2,
                                  seg_size=11, n_segments=4,
                                  hidden=64, layers=1, dropout=0.2).to(device)
        ...
        probs_va = train_one_fold(
            ...
            x_seq_tr, rf_tr, y_eval[tr],
            x_seq_va, rf_va, y_eval[va],
    ...
    print_results_table('RF-Enhanced BiLSTM', {
    ...
    out = {
        'model': 'RF-Enhanced BiLSTM',
        'label_convention': 'y=1=failed',
        'n_folds': folds,
        'architecture': {
            'n_params': int(count_parameters(RFEnhancedBiLSTM())),
            'seq_input': '46-dim reshaped to (B, 4, 11) segments',
            'rf_input': '2-dim RF OOF probs (frozen, per-sample)',
            'fusion': 'RF probs broadcast to each segment; BiLSTM -> mean-pool -> concat RF -> FC',
        },
    ...
    print(f"  Parameters: {count_parameters(RFEnhancedBiLSTM()):,}")
    return cv


if __name__ == '__main__':
    import argparse
    p = argparse.ArgumentParser(description='RF-Enhanced BiLSTM trainer')
