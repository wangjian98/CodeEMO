"""RF-LSTM v4 trainer - 5-fold CV, failed=1 label convention."""
...
from models.rf_lstm.rf_lstm_v4_model import RFLSTMv4, count_parameters
...
def main(output_dir=None, folds=5, epochs=80, batch_size=32, patience=15, seed=42):
    if output_dir is None:
        output_dir = os.path.join(_PROJECT_ROOT, 'outputs', 'unified_compare', 'rf_lstm_v4')
    os.makedirs(output_dir, exist_ok=True)
    print("=" * 72)
    print("  RF-LSTM v4: Pre-norm + Stacked Self-Attn + LayerNorm + Residual")
    ...
    print(f"\n[Step 2] RF-LSTM v4 5-fold CV (epochs={epochs}, batch={batch_size}, patience={patience})")
    ...
    for fi, (tr, va) in enumerate(skf.split(X7, y_eval), 1):
        ...
        model = RFLSTMv4(hidden=32, n_attn_layers=2, dropout=0.2).to(device)
    ...
    print_results_table('RF-LSTM v4', {
    ...
    out = {
        ...
        'n_folds': folds,
        'architecture': {
            'n_params': int(count_parameters(RFLSTMv4())),
            'design': 'Pre-norm BiLSTM + 2x Pre-norm Self-Attn + LayerNorm + Residual',
    ...
    print(f"  Parameters: {count_parameters(RFLSTMv4()):,}")
    return cv
