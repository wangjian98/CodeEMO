"""RF-LSTM v3 trainer - simplified version of v2."""
...
from models.rf_lstm.rf_lstm_v3_model import RFLSTMv3, count_parameters
...
def make_rf_oof(X7, y, n_splits=5, seed=42):
    n = len(y); oof = np.zeros((n, 2), dtype=np.float32)
...
def train_one_fold(model, optimizer, scheduler, device,
    ...
    rf_va_va = torch.tensor(rf_va, dtype=torch.float32, device=device)
...
def main(output_dir=None, folds=5, epochs=80, batch_size=32, patience=15, seed=42):
    if output_dir is None:
        output_dir = os.path.join(_PROJECT_ROOT, 'outputs', 'unified_compare', 'rf_lstm_v3')
    os.makedirs(output_dir, exist_ok=True)
    print("=" * 72)
    print("  RF-LSTM v3: simplified + self-attention + RF skip")
    ...
    print("\n[Step 1] Computing OOF RF probs...")
    rf_probs = make_rf_oof(X7, y_eval, n_splits=folds, seed=seed)
    print(f"\n[Step 2] RF-LSTM v3 5-fold CV")
    ...
    for fi, (tr, va) in enumerate(skf.split(X7, y_eval), 1):
        ...
        model = RFLSTMv3(hidden=32, dropout=0.2).to(device)
    ...
    print_results_table('RF-LSTM v3', {
    ...
    out = {
        ...
        'n_folds': folds,
        'architecture': {
            'n_params': int(count_parameters(RFLSTMv3())),
            'bi_lstm': '1-layer, hidden=32, bidirectional',
            'self_attention': '4-head, dropout=0.2 on LSTM output',
            'fusion': 'mean-pool -> concat RF probs -> Linear',
    ...
    print(f"  Parameters: {count_parameters(RFLSTMv3()):,}")
    return cv
