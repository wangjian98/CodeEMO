#!/usr/bin/env python3
"""
M-AAE-Net Slim: Dual Head + KD only (no gate, no focal-heavy, no contrastive)
==============================================================================
保留 M-AAE-Net 验证有效的 2 个创新:
  - Dual Head (hard CE + soft focal, 0.4:0.6 测试融合 → AUC +1pp)
  - KD from RF_7dim (T=3.0)

去掉 (3 个):
  - Class-aware Gating (过拟合, full 版崩盘)
  - Focal Loss 重度使用 (CW-HDM-Net 失败)
  - InfoNCE (M-AAE-Net simplified 中没效, CW-HDM-Net 也没效)

基础架构 = HDM-Net v2 (3 视图 + PIG).
"""
...
class MAASlim(nn.Module):
    """HDM-Net v2 + Dual Head (no gate, no contrastive)"""
    ...
    def forward(self, x_tree, tree_probs, x_seq, x_att):
        ...
        return {'hard_logit': hard_logit, 'soft_logit': soft_logit, 'h_fused': h_fused}
...
def focal_loss_simple(logits, labels, alpha=0.7, gamma=2.0):
    """Focal Loss (no smoothing, simplest)"""
    p = torch.sigmoid(logits)
    p_pos = torch.where(labels == 1, p, 1 - p)
    alpha_w = torch.where(labels == 1, alpha, 1 - alpha)
    focal_w = (1 - p_pos) ** gamma
...
def train_one_fold(x_tree_tr, tree_probs_tr, x_seq_tr, x_att_tr, y_tr,
    ...
    model = MAASlim(d=32, tree_depth=3, tree_width=64).to(device)
    ...
    for ep in range(1, epochs + 1):
        ...
        for i in range(0, n_tr, batch_size):
            ...
            loss_soft = focal_loss_simple(soft_logit, lt, alpha=0.7, gamma=2.0)
            loss_kd = kd_loss(hard_logit, tp[:, 1], T=3.0)

            # 0.4 hard + 0.4 soft + 0.2 KD
            loss = 0.4 * loss_hard + 0.4 * loss_soft + 0.2 * loss_kd
...
def predict_probs(model, x_tree, tree_probs, x_seq, x_att, device='cuda', batch_size=64):
    ...
    with torch.no_grad():
        for i in range(0, len(x_tree), batch_size):
            ...
            # 0.4:0.6 融合 (M-AAE-Net full 验证有效)
...
def main(output_dir='outputs/unified_compare/m_aaenet_slim', epochs=80, batch_size=32):
    set_seed(42)
    ...
    report = {
        'model': 'M-AAE-Net Slim',
        'n_samples': n,
        'n_failed': int(y.sum()),
        'n_passed': int((1-y).sum()),
        'n_params': sum(p.numel() for p in MAASlim().parameters()),
        'elapsed_seconds': elapsed,
        'architecture': {
            'base': 'HDM-Net v2 (3 views + PIG)',
            'kept_1': 'Dual Head (hard CE + soft focal)',
            'kept_2': 'KD from RF_7dim (T=3.0, w=0.2)',
            'test_fusion': '0.4 hard + 0.6 soft (verified effective in M-AAE-Net full)',
            'removed_1': 'Class-aware Gating (full 过拟合)',
            'removed_2': 'InfoNCE contrastive (simplified 验证无效)',
            'removed_3': 'Heavy Focal Loss + Class Weight (CW-HDM-Net 验证无效)',
            'removed_4': 'Stochastic View Dropout (CW-HDM-Net 验证无效)',
        },
        'cv_results': cv,
    ...
    print(f"\n{'='*72}", flush=True)
    print(f"M-AAE-Net Slim CV 结果:", flush=True)
