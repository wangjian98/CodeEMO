"""
CSEM-Net 训练脚本 - 5-fold CV, failed=1 口径, Focal Loss + LW-Fusion
"""
...
from models.csem_net.model import CSEMNet, count_parameters
...
def bce_loss(logits, target):
    """Vanilla BCE loss without pos_weight (v2: avoid degenerate solutions)."""
...
def main(output_dir=None, folds=5, epochs=80, batch_size=32, patience=10,
         seed=42):
    if output_dir is None:
        output_dir = os.path.join(_PROJECT_ROOT, 'outputs', 'unified_compare', 'csem_net')
    os.makedirs(output_dir, exist_ok=True)

    print("=" * 72)
    print(f"  CSEM-Net: Code-Study Engagement Mixture Network  (BCE-only v2)")
    ...
    print(f"\n[Step 2] CSEM-Net 5-fold CV (epochs={epochs}, batch={batch_size}, patience={patience})")
    ...
    print_results_table(f'CSEM-Net (v2 BCE)', {
    ...
    out = {
        ...
        'n_folds': folds,
        'architecture': {
            'n_params': int(count_parameters(CSEMNet())),
            'streams': ['CNN-Attention on 7-dim', 'BiLSTM on 46-dim', 'Cross-Feature MLP'],
            'fusion': 'concat(3 streams x 32-dim) -> MLP(96->32->1)',
            'loss': f'Vanilla BCE + L2(1e-2) [v2 simplified]',
        },
    ...
    print(f"  Parameters: {count_parameters(CSEMNet()):,}")
    return cv


if __name__ == '__main__':
    import argparse
    p = argparse.ArgumentParser(description='CSEM-Net unified trainer')
