"""
Mamba-SSM CPU 完整训练流程 (6步)

使用 SimplifiedMambaStudent (d_model=32, n_layers=2) 在CPU上运行完整的6步流程:

  Step 1: 数据预处理 - 7维事件编码
  Step 2: Mamba 预训练 - 下一事件预测 (自监督, 2 epochs)
  Step 3: 多尺度特征提取
  Step 4: 原型发现 - K-Means 聚类
  Step 5: 预测微调 - 5折交叉验证 (冻结骨干, 仅训练risk_head, 5 epochs/折)
  Step 6: 可解释性分析

用法:
    python models/mamba/train_cpu.py
    python models/mamba/train_cpu.py --folds 10
    python models/mamba/train_cpu.py --output-dir outputs/mamba
"""
...
from models.mamba.model import SimplifiedMambaStudent, create_model
...
def main():
    parser = argparse.ArgumentParser(description='Mamba-SSM CPU 训练流程')
    ...
    print("=" * 60)
    print("  Mamba-SSM CPU 训练流程")
    print("  模型: SimplifiedMambaStudent (d_model=32, n_layers=2)")
    print("=" * 60)
    print(f"  设备: {device}")
    ...
    pretrained_state = pretrain(
        ...
        epochs=args.pretrain_epochs, batch_size=16
    )
    ...
    representations, repr_labels = extract_representations(
        model, samples, device, batch_size=16
    )
    ...
    print_results_table("Mamba-SSM (CPU)", summary)
    ...
    # 创建并加载模型进行可解释性
    interp_model = SimplifiedMambaStudent(
        n_event_types=7, d_model=32, n_layers=2, d_state=8,
        n_prototypes=4, max_seq_len=2000
    )
    ...
    # 打印综合总结
    print("\n" + "=" * 60)
    print("  Mamba-SSM CPU 训练完成 - 综合总结")
    print("=" * 60)
    print(f"  模型: SimplifiedMambaStudent")
