...
class FullMambaStudent(nn.Module):
    ...
    def __init__(self, n_event_types=7, d_model=64, n_layers=6, d_state=16,
        ...
        # Step 1: 事件编码
        self.event_embed = nn.Embedding(n_event_types, 32)
        ...
        # Step 2: Mamba 骨干 (完整版) - d_model 加宽到 96
        self.mamba = MambaEncoder(d_model=d_model, n_layers=n_layers, d_state=d_state)
        self.final_norm = RMSNorm(d_model)
        ...
        for m in self.risk_head.modules():
    ...
    def _behavior_features(self, batch):
        """
        3 个领域先验的 "努力模式" 特征:
          - deadline_proximity: 接近 deadline 的活动占比 (越低越危险)
          - activity_diversity: 不同 part 的覆盖度 (过低=学习面窄)
          - submission_regularity: time_intervals 的稳定性 (越大越规律)
        这 3 个特征在长序列中很难自动学到, 直接喂给 risk_head
        """
        deadline_dists = batch['deadline_dists']         # (B, L)
        time_intervals = batch['time_intervals']         # (B, L)
        part_ids = batch.get('part_ids', torch.ones_like(deadline_dists, dtype=torch.long))

        B = deadline_dists.shape[0]

        # 1) 近 deadline 占比: 小于 0.2 视为贴近 ddl
        near_ddl_ratio = (deadline_dists < 0.2).float().mean(dim=-1)

        # 2) activity diversity: 覆盖的 part 数 / 7
        part_ids_f = part_ids.float()
        diversity = torch.zeros(B, device=deadline_dists.device)
        for b in range(B):
            valid = part_ids_f[b][part_ids_f[b] > 0]
            if valid.numel() == 0:
                diversity[b] = 0.0
            else:
                diversity[b] = torch.unique(valid).numel() / 7.0

        # 3) submission regularity: 1 - CV(time_intervals), 越大越规律
        ti = time_intervals.clamp(min=1e-3)
        cv = ti.std(dim=-1) / ti.mean(dim=-1).clamp(min=1e-3)
        regularity = (1.0 - cv.clamp(0, 5) / 5.0)

        feats = torch.stack([near_ddl_ratio, diversity, regularity], dim=-1)  # (B, 3)
        feats = torch.nan_to_num(feats, nan=0.5)
        return feats
    ...
    def forward(self, batch, return_repr=False):
        ...
        # Step 3: 多尺度特征 + SE 通道注意力
        multi_scale_repr = self._multi_scale_extract(mamba_out, part_ids)
        se_w = self.se_gate(multi_scale_repr)
        multi_scale_repr = multi_scale_repr * se_w
        ...
        # Step 5: 预测 (concat 手工特征让 head 看到努力模式先验)
        behavior_feats = self._behavior_features(batch)
        combined = torch.cat([multi_scale_repr, proto_repr, behavior_feats], dim=-1)
        combined = self.dropout(combined)
        risk_pred = self.risk_head(combined)
