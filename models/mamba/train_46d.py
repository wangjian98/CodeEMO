...
输入: 46d 手工特征 → reshape (46,1) → Mamba 处理 → pool → sigmoid → P(passed=1)
输出: probs/labels/fold_idx.npy (labels 跟 B46 一致: y=1=passed, 融合时需 1-probs 翻转)

用法:
    python models/mamba/train_46d.py
    python models/mamba/train_46d.py --folds 5 --output-dir outputs/mamba_46d
"""
...
from models.mamba.model_46d import create_model
...
def main():
    parser = argparse.ArgumentParser(description='Mamba-46d 训练')
    ...
    parser.add_argument('--output-dir', type=str, default='outputs/mamba_46d')
    ...
    print_results_table("Mamba 46-dim", summary)
    ...
    with open(os.path.join(args.output_dir, 'results.json'), 'w') as f:
        json.dump({
            'model': 'Mamba 46-dim (简化版: reshape→Mamba)',
            'config': {'input_dim': 46, 'd_model': 48, 'n_layers': 4, 'd_state': 12,
                       'epochs': args.epochs, 'batch_size': args.batch_size,
                       'patience': args.patience, 'lr': args.lr,
                       'device': str(device)},
            'cv_results': {
