#!/usr/bin/env python3
"""
M-AAE-Net v2 (Simplified): HDM-Net v2 + KD + InfoNCE only
================================================================
回退双门控 + 双 head, 保留 2 个验证有效的创新:
  - KD from RF_7dim (AUC 提升)
  - InfoNCE 对比损失 (passed 召回提升)

架构 = HDM-Net v2 + 96-d 嵌入(用于对比) + 蒸馏信号
"""
...
# ============================================================
# 简化模型: M-AAE-Net v2
# - 复用 HDM-Net v2 的 3 视图 + PIG
# - 加 96-d 嵌入用于对比损失
# - 加 KD 蒸馏
# ============================================================
class MAESimple(nn.Module):
    """HDM-Net v2 + 嵌入投影 + 标准 head"""
    def __init__(self, d=32, tree_depth=3, tree_width=64, dropout=0.3):
        ...
        # PIG (per-instance gating, 与 HDM-Net v2 一致)
    ...
    def forward(self, x_tree, tree_probs, x_seq, x_att):
        ...
        # 标准 head
        logit = self.head(h_fused).squeeze(-1)
...
def train_one_fold(x_tree_tr, tree_probs_tr, x_seq_tr, x_att_tr, y_tr,
    ...
    model = MAESimple(d=32, tree_depth=3, tree_width=64).to(device)
    ...
    for ep in range(1, epochs + 1):
        ...
        for i in range(0, n_tr, batch_size):
            ...
            loss_bce = bce(logit, yt)
            loss_info = infonce_loss(embed, lt, temperature=0.1)
            loss_kd = kd_loss(logit, tp[:, 1], T=3.0)

            # 0.7 BCE + 0.2 InfoNCE + 0.1 KD
            loss = 0.7 * loss_bce + 0.2 * loss_info + 0.1 * loss_kd
...
def main(output_dir='outputs/unified_compare/m_aaenet', epochs=80, batch_size=32):
    set_seed(42)
    ...
    report = {
        'model': 'M-AAE-Net (simplified)',
        'n_samples': n,
        'n_failed': int(y.sum()),
        'n_passed': int((1-y).sum()),
        'n_params': sum(p.numel() for p in MAESimple().parameters()),
        ...
        'architecture': {
            ...
            'fusion': 'PIG (per-instance gating, same as HDM-Net v2)',
            'innovation_kept_1': 'InfoNCE Loss (96-d embedding, temp=0.1, weight=0.2)',
            'innovation_kept_2': 'KD from RF_7dim (T=3.0, KL weight=0.1)',
            'innovation_removed_1': 'Class-aware Gating (过拟合)',
            'innovation_removed_2': 'Dual Head (过拟合)',
            'innovation_removed_3': 'Focal Loss (过度预测 failed)',
        },
        'cv_results': cv,
    ...
    print(f"\n{'='*72}", flush=True)
    print(f"M-AAE-Net (simplified) CV 结果:", flush=True)
