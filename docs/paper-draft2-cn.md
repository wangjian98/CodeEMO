...
2. 在 `X_train_46d` 上训练 LSTM-MLP → 在 `X_val_46d` 上获得 OOF 预测。
...
| LSTM-MLP（46 维） | 0.8289 ± 0.031 | 0.8981 ± 0.017 | 0.8377 ± 0.051 | 0.8659 ± 0.028 | 0.9068 ± 0.020 |
...
| LSTM-MLP（46 维） | 0.8289 | 0.8981 | 0.8377 | 0.8659 | 0.9068 |
...
**L5 — LSTM-MLP-46d 与 LSTM-Seq-7d 的机制差异。** 我们的 LSTM-MLP-46d 专家实现为作用于 46 维手工特征向量的单步门控 MLP（seq_len = 1），而 LSTM-Seq-7d 基线（在更广泛的 CodeEMO 项目 [21] 中使用）是真实事件序列模型（max_seq_len = 500，截断）。因此两个 LSTM **并非直接可比**(一个 gated MLP,一个事件序列模型)：一个通过门控利用非线性特征交互，另一个利用事件序列上的时序递归。尽管存在这种机制差异，我们在实验表中仍都标记为 "LSTM" 基线，以保持与项目仓库的术语一致性。未来工作应明确区分这两种机制。
