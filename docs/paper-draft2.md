...
Route B employs a single-layer LSTM with hidden dimension 32, trained on the 46-dimensional hand-crafted features (reshaped as a single-step sequence). The LSTM expert captures interactions among the richer feature set through its gating mechanisms. The LSTM expert alone achieves Accuracy = 0.8289, Precision = 0.8981, Recall = 0.8377, F1 = 0.8659, AUC = 0.9068.
...
1. **Single-model baselines:** RF on 7-dim, LSTM on 46-dim, LSTM on 7-dim, RF on 46-dim.
