# HDM-Net v2：基于 PIG 融合的多视图混合架构及其在编程学习者风险早期识别中的应用
...
**方法。** 本文提出 **HDM-Net v2**，一种含 **33,220 个参数** 的**四分支架构**：（1）**树分支**——将 7 维原始事件计数与 2 维随机森林折外概率拼合后送入可配置 MLP；（2）**序列分支**——将 46 维手工特征向量 reshape 为 46 步单变量序列后输入双向 LSTM；（3）**注意力分支**——将 7 类事件 reshape 为长度 7 的序列，输入含 LayerScale 的预归一 Transformer；（4）**融合分支**——以 **PIG（逐实例门控）** 实现对三路分支嵌入的逐学生加权融合。XCA（跨视图交叉注意力）最初在设计中存在，但经验上在 *n* = 473 数据集上劣于 PIG-only（ΔF1 = −0.0094），已从默认配置中移除，相关讨论见 §5.3。在 CS1 MOOC 数据集（*n* = 473，159 通过 / 314 失败，2 850 万条事件）上以 5 折分层交叉验证评估，标签约定 `y = 1 ⇒ failed`。

**结果。** HDM-Net v2 取得 **F1 = 0.8982 ± 0.022**、**AUC = 0.9273 ± 0.014**，在五项主要指标上均超过最强 7 维随机森林基线（F1 = 0.8876 ± 0.019、AUC = 0.9175 ± 0.012），且 F1 与 AUC 之差均超过 0.5 个标准差。消融实验表明**四个分支缺一不可**，其中**树分支**贡献最大。
...
1. **HDM-Net v2。** 本文设计了一种**四分支多视图架构**，共 **33,220 参数**：
   ...
   - **分支 4 — 融合分支（PIG）：** 三路分支嵌入经 3 路 softmax 逐实例门控（PIG）直接加权融合 → 32 维融合嵌入 → 线性头 → logit。XCA（跨视图交叉注意力）作为可选扩展存在，详见 §3.3.4 注释与 §5.3 局限性分析。
...
3. **四项分支均不可或缺的消融证据。** 任意去掉一个分支均使 F1 下降；**树分支**贡献最大（去掉后 ΔF1 = −0.0699，Seq 为 −0.0133，Attn 为 −0.0092）。
...
跨注意力在视觉-语言（CLIP [37]）、多模态推荐以及图学习（GAT [38]、HAN [39]）中得到广泛使用。近期的"XCA 风格"设计在融合前对多个特征视图执行跨注意力。**本工作最初在 EDM 场景下尝试了 XCA（见 §3.3.4 与 §5.3），但经验上劣于 PIG-only（ΔF1 = −0.0094），已转为可选扩展模块。**
...
#### 3.3.4 分支 4 — 融合分支（PIG，XCA 为可选扩展）

融合分支的核心阶段是 **PIG（逐实例门控）**——作用于三个 XCA 增强后的分支嵌入。**XCA（跨视图交叉注意力）** 作为可选扩展被记录（其从默认配置中移除的经验依据见 §5.3）。

**XCA（可选扩展，不在默认配置中）。** 若启用，对每对分支 (i, j) ∈ {(tree, seq), (tree, attn), (seq, attn)} 计算对第一分支的跨注意力增强：
...
将 XCA 增强后的嵌入拼接、线性投影回 $d = 32$，送入 PIG。**实际部署中 XCA 默认禁用（详见 §5.3）。**
...
**四个分支缺一不可。****树分支**单一移除时贡献最大（ΔF1 = −0.0699）；注意力分支贡献最小（ΔF1 = −0.0092）；序列分支居中（ΔF1 = −0.0133）。
...
**多视图互补性。** RF_7dim 仅看 7 维原始事件计数。HDM-Net v2 还接收：（a）通过 BiLSTM 处理的 46 维手工特征序列；（b）通过 Transformer + 位置嵌入处理的 7 维事件类型序列；（c）回注至树分支的随机森林折外概率。三个分支贡献质性不同的归纳偏置，经 PIG 融合后能提取比单一视图更多的信号。

**逐实例路由。** PIG 按学生重新加权三路原始分支嵌入。XCA（跨注意力）模块在启用时允许每个分支 attend 到其他两个分支，捕获**视图间依赖**——这是独立分支 MoE 无法建模的，但**经验上在 *n* = 473 数据集上劣于 PIG-only（ΔF1 = −0.0094，详见 §5.3），故已从默认配置中移除**。
...
| XCA — 跨视图交叉注意力 *(可选扩展；不在默认配置中)* | [10] 跨注意力；[37] 多视图融合（CLIP 风格） | 在三个行为嵌入视图之间做两两交叉注意力；**经消融后默认禁用**（详见 §5.3） |
| PIG — 逐实例门控 | [19, 20, 21, 22] MoE 习得门控 | 在 33K 参数预算下，对三路原始分支嵌入进行 3 路 softmax 逐实例门控 |
| 在 33K 参数下联合训练四分支 | [14, 15, 16] 加权集成；[17, 18] 堆叠 | 以端到端方式联合训练、显式建模跨视图交互 |

**创新所在。** 本文并不在某一单独组件上创新——LSTM、Transformer、跨注意力、MoE 门控、树蒸馏均已有丰富前期工作。**创新之处在于**（一）将三种特征类别与一种逐实例门控相结合的整体架构（四分支、33,220 参数）；（二）**经过严格消融验证**——XCA 与 Focal-Loss 等常见做法均被证实在 *n* = 473 数据集上反作用，故从默认配置中剔除。最终架构在 CS1 数据集上以参数高效的方式取得当前最优 F1。
...
**L7 — XCA 已从默认配置移除。** 跨视图交叉注意力原本包含于融合分支中，但经验上劣于 PIG-only（在 *n* = 473 下 ΔF1 = −0.0094；详见 §5.3）。默认 `hdm_net_v2` 因此**不含 XCA 模块**。未来在更大样本（*n* > 2,000）上的工作可重新审视跨视图交互是否在大规模下变得有益。
...
本文提出 **HDM-Net v2**，一种基于 **PIG 融合的四分支多视图混合架构**（其中 XCA 跨注意力为可选扩展，已从默认配置中移除），用于从编程行为日志中早期识别高风险学生。HDM-Net v2 集成了树分支（7 维事件计数 + RF 折外概率）、序列分支（46 维特征上的 BiLSTM）、注意力分支（带 LayerScale 的预归一 Transformer）和融合分支（XCA 跨注意力 + PIG 逐实例门控），共 **33,220 参数**。

在 CS1 MOOC 数据集（*n* = 473，5 折分层 CV）上，HDM-Net v2 取得 **F1 = 0.8982 ± 0.022**、**AUC = 0.9273 ± 0.014**，在五项主要指标上均超过最强 7 维随机森林基线，F1 与 AUC 之差均超过 0.5 σ。消融实验证实**四个分支均不可或缺**，**树分支**单一贡献最大。PIG 路由分布呈现近乎均衡的平均权重与显著的学生间方差，证明习得的是个性化路由而非全局常数。
...
| PIG 融合（concat + MLP 门控 + 线性头） | ~4,000 |
| **合计** | **约 33,220** |
...
全部实现代码（含四分支 HDM-Net v2 架构、可选 XCA 跨注意力扩展模块、PIG 门控模块、消融评测管线、评估工具与配置文件）以 MIT 协议发布于：
...
**图 1 — HDM-Net v2 四分支架构。** 分支 1（TreeHead）将 7 维事件计数 + 2 维 RF 折外概率送入 MLP；分支 2（SeqBranch）以 1 层 BiLSTM 处理 reshape 为 46 步序列的 46 维特征；分支 3（AttnBranch）以 2 层预归一 Transformer + LayerScale 处理 reshape 为 7 步序列的事件类型；分支 4（融合）以 PIG 3 路 softmax 逐实例门控直接产生融合嵌入，最终由线性头映射至 logit。（XCA 两两跨视图交叉注意力作为可选扩展存在，详见 §3.3.4 与 §5.3。）**总参数量：33,220**。
